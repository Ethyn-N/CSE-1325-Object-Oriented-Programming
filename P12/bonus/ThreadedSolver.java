...
public class ThreadedSolver {
    public static void main(String[] args) {
        ...
        for(var file : polynomials.keySet()) {
            double temp_min = min;
            double temp_max = max;
            int temp_threads = threads;
            double temp_slices = slices;
            double temp_precision = precision;

            Polynomial p = polynomials.get(file);

            Thread thread = (new Thread( () -> p.solve(temp_min, temp_max, temp_threads, temp_slices, temp_precision)));
            thread.start();
            try {
                thread.join();
